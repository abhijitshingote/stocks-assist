/* Strong Stocks.
   Universe: liquid names with a current TI65.
   Sort toggle: mcap-adjusted TI65 vs raw TI65 vs monthly setup/readiness
   (setupParts from vsg-90d, as-is). Biotech exclude chip like 90d.
...
(function () {
    ...
    function scoreChipHtml(s) {
        ...
        if (sortMode === 'raw') {
            const v = s.ti65;
            if (v == null) return '';
            return `<span class="vsg-score" title="raw TI65 ${v.toFixed(2)}">${v.toFixed(2)}</span>`;
        }
        const v = s.adjusted_ti65;
        if (v == null) return '';
        return `<span class="vsg-score" title="adjusted TI65 ${v.toFixed(2)} = ` +
            `(ti65−1) / (0.120 × (clip(mcap,$500M,$100B)/$100B)^-0.151)">${v.toFixed(2)}</span>`;
    }
    ...
    function listValueFn(s) {
        ...
        return {
            ...
            cls: v != null ? (v >= 1 ? 'positive' : 'negative') : 'muted',
        };
    }
    ...
    const excludeHost = document.getElementById('ssExcludes');
    if (excludeHost) {
