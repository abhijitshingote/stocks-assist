...
def compute_and_load_metrics(connection):
    ...
    SELECT 
        ...
        ROUND((pc.current_close * pc.today_volume)::numeric, 0) as dollar_volume,
        ROUND(va.avg_vol_10d::numeric, 2) as avg_vol_10d,
        ROUND((pc.today_volume::numeric / NULLIF(va.avg_vol_10d, 0)), 2) as vol_vs_10d_avg,
        pc.dr_1,
        pc.dr_5,
        pc.dr_20,
        pc.dr_60,
        ...
        ROUND((pc.current_close / NULLIF(tf.eps_ttm,0))::numeric, 2) AS pe,
        ROUND((t.market_cap / NULLIF(tf.revenue_ttm,0))::numeric, 2) AS ps_ttm,
        ROUND((pc.current_close / NULLIF(fe.eps_avg,0))::numeric, 2) AS fpe,
        ROUND((t.market_cap / NULLIF(fe.revenue_avg,0))::numeric, 2) AS fps,
        ROUND(((fe.eps_avg - tf.eps_ttm) / NULLIF(ABS(tf.eps_ttm), 0) * 100)::numeric, 2) AS eps_growth,
        ROUND(((fe.revenue_avg - tf.revenue_ttm) / NULLIF(tf.revenue_ttm, 0) * 100)::numeric, 2) AS revenue_growth,
        rs.rsi,
        rs_mc.rsi_mktcap,
