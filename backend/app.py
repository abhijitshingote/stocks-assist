...
def get_abi_watchlist_data():
    """Watchlist membership with stock_metrics rows (same source as /abi-trades)."""
    wl = _load_watchlist()
    comments = _load_abi_ticker_notes()
    ...
    try:
        ...
        for row in rows:
            t = row.get('ticker')
            got.add(t)
            row['watchlist_notes'] = (comments.get(t) or {}).get('notes', '')
            row['watchlist_added_at'] = (wl.get(t) or {}).get('added_at', '')
        for t in tickers:
            if t in got:
                continue
            rows.append({
                'ticker': t,
                'watchlist_notes': (comments.get(t) or {}).get('notes', ''),
                'watchlist_added_at': (wl.get(t) or {}).get('added_at', ''),
            })
        return jsonify(rows)
    except Exception as e:
