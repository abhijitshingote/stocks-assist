...
| **VolspikeGapper** `VolspikeGapper-*` | `spike_day_count > 0 OR gapper_day_count > 0`; order `last_event_date DESC`. Optional `?lookback_days=N` keeps `last_event_date >= max(OHLC.date) − N` calendar days. `last_event_type` = `volume_spike` \| `gapper` \| `both`. `last_event_magnitude` = event-day vol ratio (always). `last_event_return` = close/prev−1 (always). Adds `adjusted_event_return = last_event_return×100 / (clip(mcap,$200M,$100B)/$100B)^-0.134` |
